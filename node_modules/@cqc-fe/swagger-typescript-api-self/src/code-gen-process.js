...
class CodeGenProcess {
  ...
  async start() {
    ...
    const generatedFiles = files.map((file) => {
      ...
      if (file.tag) this.fileSystem.createDir(`${this.config.output}/api/${file.tag}`)

      if (this.fileSystem.pathIsExist(`${this.config.output}/http-client.ts`) && this.fileSystem.pathIsExist(`${this.config.output}/http-client-types.ts`) && file.name.indexOf('http-client') > -1) return;

      if (this.config.toJS) {
        this.fileSystem.createFile({
          path: file.tag ? `${this.config.output}/api/${file.tag}` : this.config.output,
          fileName: file.name,
  ...
  getNewUsageComponentSchemas(){
    const newUsageComponentSchemas = this.schemaComponentMap.filter("schemas").map(item => ({
      ...item,
      typeName: this.typeName.format(item.typeName)
    }))
    newUsageComponentSchemas.map(item => {
      const typeName = item.typeName;
      item.parent = []
      newUsageComponentSchemas.find(schema => {
        const properties = schema.rawTypeData.properties
      if(properties){
        Object.keys(properties).map(key => {
          if(properties[key].$parsed){
            if(properties[key].$parsed.content.indexOf(typeName) > -1){
              item.parent.push(schema.typeName)
            }
            if(properties[key].$parsed.items && properties[key].$parsed.items.$parsed.content.indexOf(typeName) > -1){
              item.parent.push(schema.typeName)
            }
            item.parent = Array.from(new Set(item.parent))
          }
        })
      }
      })
    })
    return newUsageComponentSchemas
  }


  getParentTypeName(name, usageComponentSchemas) {
    const findData = usageComponentSchemas.find(item => item.typeName === name);

    if (findData && findData.parent && findData.parent.length > 0) {
        return findData.parent.flatMap(item => {
            // 添加终止条件，避免无限递归
            if (item === name) {
                return [name];
            }
            return this.getParentTypeName(item, usageComponentSchemas);
        });
    } else {
        return [name];
    }
  ...
  prepareModelType = (typeInfo) => {
    ...
    let moduleName = []
    const usageComponentSchemas = this.getNewUsageComponentSchemas()

    const parentTypeName = Array.from(new Set(this.getParentTypeName(name, usageComponentSchemas).flat(Infinity)));


    this.schemaRoutes.getGroupedRoutes().combined && this.schemaRoutes.getGroupedRoutes().combined.forEach(item => {
      item.routes.map(sub => {
        if ((sub.response.type && parentTypeName.includes(sub.response.type)) || (sub.request && sub.request.payload && parentTypeName.includes(sub.request.payload.type)) || sub.request && sub.request.requestParams && parentTypeName.includes(sub.request.requestParams.typeName)) {
        ...
        ..._.reduce(
          ...
          (apiFileInfos, route) => {
            ...
            if (generateClient) {
              ...
              const dataModuleContent = this.templates.renderTemplate(templatesToRender.dataContracts, {
                ...configuration,
                route,
              });
              apiFileInfos.push(
                this.createOutputFileInfo(configuration, 'index', apiModuleContent, pascalCase(route.moduleName)),
              );
              apiFileInfos.push(
                this.createOutputFileInfo(configuration, 'type', dataModuleContent, pascalCase(route.moduleName)),
              );
            }
  ...
  createOutputFileInfo = (configuration, fileName, content, tag) => {
